...
func (srv *TgService) SendAnimArticleHTML(steps_id string, chat_id, milisecond int) error {
	steps, ok := srv.Articles[steps_id]
	if !ok {
		return nil
	}
	...
	for i, v := range steps {
		...
		if i == 0 {
			SendMessageResp, err := srv.SendMessageHTML(chat_id, v)
			if err != nil {
				return fmt.Errorf("SendAnimArticleHTML SendMessageHTML err: %v", err)
			}
			messId = SendMessageResp.Result.MessageId
			continue
		}
...
func (srv *TgService) ReplaceHtmlTag(htmlStr string) string {
	htmlStr = strings.Replace(htmlStr, "|<", "<b>", -1)
	htmlStr = strings.Replace(htmlStr, ">|", "</b>", -1)
	htmlStr = strings.Replace(htmlStr, "_<", "<u>", -1)
	htmlStr = strings.Replace(htmlStr, ">_", "</u>", -1)
	return htmlStr
}

func (srv *TgService) CloseHtmlTag(htmlStr string) string {
	var prePre string
	var pre string
	var tag string
	for i, v := range htmlStr {
		if i < 3 {
			prePre = pre
			pre = string(v)
			continue
	...
	if tag != "" {
		htmlStr = fmt.Sprintf("%s</%s>", htmlStr, tag)
	}
	htmlStr = strings.Replace(htmlStr, "•", "", -1)
	return htmlStr
}

func (srv *TgService) SendAnimArticleHTMLV2(steps_id string, chat_id, milisecond int) error {
	time.Sleep(2000)
	steps := srv.Articles[steps_id][0]
	steps = srv.ReplaceHtmlTag(steps)
	...
	for i, v := range steps {
		...
		if cnt > 1 && (string(v) == "." || string(v) == "," || string(v) == ":" || string(v) == "!" || string(v) == "?" || string(v) == "\n" || string(v) == ")") {
			if !isSendedMess {
				SendMessageResp, err := srv.SendMessageHTML(chat_id, string(textRune))
				if err != nil {
					srv.l.Error(fmt.Errorf("SendAnimArticleHTMLV2 SendMessageHTML err: %v", err))
					return nil
				}
				messId = SendMessageResp.Result.MessageId
				isSendedMess = true
			...
			if err != nil {
				srv.l.Error(fmt.Errorf("SendAnimArticleHTMLV2 EditMessageTextHTML err: %v", err))
				// return nil
			}
		}
	}

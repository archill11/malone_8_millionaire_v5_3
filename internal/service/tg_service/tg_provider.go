...
func (srv *TgService) SendVideoWCaption(chat_id int, caption, fileNameInServer string) (models.SendMediaResp, error) {
	...
	if err != nil {
		return models.SendMediaResp{}, fmt.Errorf("SendVideoWCaption SendPhoto err: %v", err)
	}
	return resp, nil
}

func (srv *TgService) SendVideoWCaptionWRM(chat_id int, caption, fileNameInServer, reply_markup string) (models.SendMediaResp, error) {
	futureJson := map[string]string{
		"chat_id":    strconv.Itoa(chat_id),
		"caption":    caption,
		"parse_mode": "HTML",
	...
	contentType, body, err := files.CreateForm(futureJson)
	if err != nil {
		return models.SendMediaResp{}, fmt.Errorf("SendVideoWCaptionWRM CreateForm err: %v", err)
	}
	resp, err := srv.SendVideo(body, contentType)
	if err != nil {
		return models.SendMediaResp{}, fmt.Errorf("SendVideoWCaptionWRM SendPhoto err: %v", err)
	}
	return resp, nil
}
